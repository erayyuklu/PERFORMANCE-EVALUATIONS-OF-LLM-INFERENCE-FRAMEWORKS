...
- **GPU utilization stays ≈89–100%** across every cell. The L4 is fully saturated, so the compute cost of dequantization shows up directly in TPOT/ITL.
...
The decisive view: on the **throughput axis** every TurboQuant variant lands well below the FP8 baseline (1,097 tok/s) while accuracy is — at best — only marginally above on GSM8K and uniformly ~2 pp lower on ARC. On the **P95 TTFT axis under saturation** TurboQuant variants actually beat the baseline (the smaller KV blocks reduce queue buildup), so they would be a defensible choice only if first-token responsiveness at u128 mattered more than overall token throughput.
...
1. **FP8 baseline wins on throughput and per-token speed.** TPOT regresses by **+28% (k8v4) to +106% (3bit_nc)** at u128, ITL roughly doubles on the 3-bit variants, and output-token throughput tops out at **760 tok/s for the best TurboQuant preset (`k3v4_nc @ u128`) vs 1,097 tok/s for fp8**.

2. **TurboQuant wins on TTFT once the GPU is saturated.** At u128 the smaller KV blocks let new requests admit faster — TTFT P50 drops 9–28% and P95 TTFT improves by an order of magnitude on `3bit_nc` (2.0 s vs 217.6 s) — but slower per-token decode cancels this on E2E for every variant except `k8v4`, which essentially ties the baseline on E2E.
...
4. **Quality loss is within statistical noise.** ARC accuracy drops by ~2 pp uniformly; GSM8K is essentially unchanged on the conservative presets and even **slightly above the baseline on `k8v4` (0.891 vs 0.879) and `4bit_nc` (0.882 vs 0.879)**. TurboQuant **does not meaningfully degrade task accuracy**, even at 3-bit keys + 3-bit values.

5. **Cost-optimal configuration remains FP8 KV @ u128 at $0.40/M tokens.** The cheapest TurboQuant cell (`k3v4_nc @ u128 = $0.58/M`) is 45% more expensive; every other TurboQuant cell sits between **$0.76 and $1.50 per million tokens**.

6. **GPU utilization is ≈89–100% everywhere** — the L4 is the bottleneck. The added compute from dequantization manifests directly as slower decoding rather than as memory headroom.
...
| **Best TTFT under saturation** | `turboquant_k3v4_nc` (TTFT P50 ~28% faster than fp8 @ u128) or `turboquant_3bit_nc` (P95 TTFT 2.0 s vs fp8's 217.6 s) — useful only when admit-time matters more than per-token speed |
